...
module.exports = class KadoUI {
  ...
  config(){
    return {
      ...
      module: {
        blog: {
          enabled: true,
          root: this.path.resolve(__dirname + '/../modules/blog/index.js')
        },
        content: {
          enabled: true,
          root: this.path.resolve(__dirname + '/../modules/content/index.js')
        },
        doc: {
          enabled: true,
          root: this.path.resolve(__dirname + '/../modules/doc/index.js')
        },
        home: {
          enabled: true,
          root: this.path.resolve(__dirname + '/../modules/home/index.js')
        },
        kado: {
          enabled: true,
          root: this.path.resolve(__dirname + '/../modules/kado/index.js')
        },
        setting: {
          enabled: true,
          root: this.path.resolve(__dirname + '/../modules/setting/index.js')
        },
        staff: {
          enabled: true,
          root: this.path.resolve(__dirname + '/../modules/staff/index.js'),
        }
      }
    }
  }
  ...
  test(config,options){
    ...
    describe(config.title || config.name,()=>{
      describe('cli',()=>{
        const app = new Kado(config)
        testModules(app,'cli')
        if(typeof options.cli === 'function'){
          options.cli(app,{})
        }
      })
      describe('admin',()=>{
        ...
        if(typeof options.admin === 'function'){
          options.admin(app,params)
        }
      })
      describe('main',()=>{
        //setup server
        ...
        it('should have a homepage',()=>{
          return request.getAsync(makeUrl('/'))
            .then((result)=>{
              ...
              expect(result.body).to.match(/<html/)
            })
        })
        describe('modules',()=>{
