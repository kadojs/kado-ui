...
window._mirrorFieldToUri = function(){
  let fromEl = $(this);
  let toEl = $('#' + fromEl.attr('data-to-uri'));
  let isEmpty = toEl.val() === '';
  console.log(fromEl.attr('data-to-uri'))
  let updateField = () => {
    let title = fromEl.val();
    let uri = title.replace(/[\s]+/g,'-').toLowerCase();
    if(isEmpty) toEl.val(uri);
  }
  fromEl.keyup(updateField);
  fromEl.change(updateField);
  fromEl.focusout(updateField);
}
