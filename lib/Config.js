...
module.exports = class Config extends ObjectManage {
  constructor(){
    super()
    this.dev = null
    this.title = 'Kado'
    ...
    this.log = {
      ...
      winston: {
        enabled: true,
        default: true,
        root: path.resolve(path.join(libRoot,'LoggerWinston.js')),
        transport: {
          console: {
            enabled: true,
              level: null
    ...
    this.render = {
      ...
      mustache: {
        enabled: false,
        root: path.resolve(path.join(libRoot,'ViewMustache.js'))
      }
    }
    //dynamic connectors
    this.connector = {
      stretchfs: {
        load: false,
          root: path.resolve(path.join(libRoot,'ConnectorStretchfs.js')),
          callback: {
          method: 'post',
    ...
    this.db = {
      ...
      sequelize: {
        enabled: false,
        load: false,
        root: path.resolve(path.join(libRoot,'DatabaseSequelize.js')),
        skipConnect: false,
        name: 'kado',
        host: '127.0.0.1',
        port: 3306,
    ...
    this.email = {
      emailjs: {
        enabled: false,
          load: true,
          root: path.resolve(path.join(libRoot,'EmailJS.js')),
          user: 'kado@localhost',
          password: '',
          host: 'localhost',
          port: null,
